...
function checkUser()
{
	...
	if (!isset($_SESSION['colorswayindia_user_id'])) {
		header('Location: http://localhost/colorswayindia/admin/login.php');
		exit;
	}
...
function doLogin()
{
	...
	if ($userName == '') {
		...
		if (dbNumRows($result) == 1) {
			...
			$_SESSION['colorswayindia_user_id'] = $row['user_id'];
...
{
	if (isset($_SESSION['colorswayindia_user_id'])) {
		unset($_SESSION['colorswayindia_user_id']);
		session_unregister('colorswayindia_user_id');
	}
